...
const fetchAllBookings = async ({ page = 1 }: { page: number }) => {
  ...
  const response = await ayoojonApi.get(`bookings/user/search/?pageNumber=${page}&status=reserved`, headers);
  return {
...
export const Accepeted = () => {
  const [page, setPage] = useState<number>(1);

  const { data, isLoading } = useQuery<IData, Error>(
    ['client-bookin-accepeted-search', page],
